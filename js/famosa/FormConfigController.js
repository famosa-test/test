// Controller for the Form Configurator
// provides data and event handlers for the default form-based configurator
angular.module( "famosa" )
	.controller('famosaFormConfigurator', 
			[ '$scope', '$http', '$rootScope', '$location', 'splotService', 
			  function($scope, $http, $rootScope, $location, splotService) {
		...
		$scope.loadModel = function ( ) {
			splotService.loadModel().then(
				function( model ) {
					$scope.$apply( function(){
						$scope.model 		= model;
						$scope.constraints	= model.constraints;
					});
				}
			);
		};
